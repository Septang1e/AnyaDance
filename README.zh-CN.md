...
驱动通过 `127.0.0.1:39570` 上的 UDP JSON 接收姿态与控制器输入帧。伴随程序 `AnyaDance.exe` 启动时发送一帧六设备初始姿态，之后仅在状态变化时发送；空闲期间驱动会保持最后接受的姿态。
...
虚拟头显默认每只眼睛以 `64x64` 渲染，为本地模型推理保留显存。高级用户无需重新构建驱动即可通过覆盖两个设置项来提高分辨率。
...
所有六个设备的 Y 值都有 `2.0 m` 的绝对硬限制（`-2.0 <= Y <= 2.0`）。UI 在序列化前钳制，原生驱动在数据包校验后再次钳制。
